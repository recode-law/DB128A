...
from .rest_api import (create_court, search_courts, court_percentage, get_court_detail, get_court_ids,
                       ...
                       get_camera_perspectives, get_conferencing_software, get_rejection_reasons,
                       CourtListLimitExceededError)
...
@group_required("Verifiziert")
@require_http_methods(["GET"])
def rest_api_court_percentage(request):
    try:
        response_data = court_percentage(request.GET)
    except KeyError as e:
        return HttpResponse(status=400, content=f"Missing parameter: {str(e)}")
    except ValueError as e:
        return HttpResponse(status=400, content=f"Invalid value provided: {str(e)}")
